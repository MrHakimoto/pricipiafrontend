...
import { Questao, ListaCompleta, Alternativa, Topico, Prova } from '@/types/list';
...
export function useListaData(listaId: string | string[]): UseListaDataReturn {
  ...
  const processarTentativaExistente = (tentativaExistente: any) => {
    ...
    console.log('📝 Respostas carregadas:', Object.keys(respostas).length, 'respostas');
  ...
  const fetchListaData = async () => {
    ...
    if (status === 'authenticated' && session?.laravelToken && listaId) {
      try {
        ...
        const { questoesData, listaInfoData } = processarDadosLista(dadosLista, idNumber);
        ...
        console.log('   - Respostas salvas:', Object.keys(tentativaData.respostasSalvas).length);
        ...
        setError(err instanceof Error ? err.message : "Falha ao carregar a lista de exercícios. Tente novamente.");
      } finally {
        setIsLoading(false);
      }
  ...
  const handleFinalizarTentativa = async (): Promise<FinalizarTentativaResponse> => {
    if (!session?.laravelToken) throw new Error("Usuário não autenticado");
    if (!resolucaoId) throw new Error('Nenhuma tentativa ativa para finalizar');
  ...
  if (questaoIncompleta) {
    console.error('⚠️ Questão ainda sem dificuldade após formatação:', questaoIncompleta);
