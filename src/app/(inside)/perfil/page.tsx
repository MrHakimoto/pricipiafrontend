...
useEffect(() => {
  const loadData = async () => {
    try {
      setIsLoading(true);
      const data = await fetchMyProfile(session?.laravelToken);
      console.log(data);
      ...
      if (data) {
        ...
        setForm({
          id: data.user_id,
          name: session?.user?.name || "", // pode vir do NextAuth
          cpf: aplicarMascaraCPF(data.cpf || ""),
          birth_date: formatarData(data.birth_date),
          gender: mapeargender(data.gender),
          celular: aplicarMascaraTelefone(data.phone || ""),
        });
      }
    } catch (error) {
  ...
  if (session?.laravelToken) loadData();
...
const handleSubmit = async (e) => {
  ...
  // Função auxiliar: converte DD/MM/AAAA → AAAA-MM-DD
  ...
  const dadosParaEnvio = {
    ...
    gender: form.gender
      ? form.gender[0].toUpperCase() // 'masculino' → 'M'
      : null,
    birth_date: converterDataParaISO(form.birth_date), // formato ISO
  };

  console.log(' Dados para envio ao backend:', dadosParaEnvio);

  try {
    const response = await saveMyProfile(dadosParaEnvio, session?.laravelToken);
    ...
    <div className="min-h-screen bg-[#00091A] text-white font-sans relative">
      ...
      <main className="flex gap-8 mx-auto p-8 max-w-5xl items-start">
        ...
        >
          ...
          >
            <svg width="94" height="93" viewBox="0 0 94 93" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <path d="M47 22.923C37.271 22.923 29.375 30.735 29.375 40.3606C29.375 49.8001 36.848 57.4726 46.765 57.7516H47.188H47.517H47.611C57.105 57.4261 64.578 49.8001 64.625 40.3606C64.625 30.735 56.729 22.923 47 22.923Z" fill="white" />
            </svg>
          </motion.div>
          <motion.p
            initial={{ opacity: 0 }}
          ...
          >
            miltonjunio9x2@gmail.com<br /><br /><br /><br /><br /><br />
          </motion.p>
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          ...
          >
            <span className="font-medium">Nível 19 | Ranking 1</span><br />
            desde 01/01/2025
          </motion.p>
        </motion.aside>
        ...
        <section className="flex-1">
          {/* Tabs */}
          <motion.div
            className="flex gap-6 border-b border-gray-700 text-sm ml-2"
            initial="hidden"
            animate="visible"
            variants={containerVariants}
          >
            {['dadosPessoais', 'dadosAcesso', 'assinaturas'].map((tab, index) => (
              <motion.button
                key={tab}
                variants={tabVariants}
                className={`cursor-pointer pb-2 ${abaAtiva === tab ? 'border-b-2 border-white text-white font-medium' : 'text-gray-400 hover:text-white'}`}
                ...
                >
                  <motion.div variants={itemVariants} className="col-span-2">
                    ...
                    <input
                      ...
                      className="w-full border border-gray-600 bg-transparent p-2 rounded text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-[#0e00d0]"
                    />
                    {errors.name && (
                      <motion.p
                        initial={{ opacity: 0 }}
                  ...
                  <motion.div variants={itemVariants}>
                    ...
                    <select
                      ...
                      value={form.gender}
                      onChange={handleChange}
                      className="w-full border border-gray-600 bg-transparent p-2 rounded text-white focus:outline-none focus:ring-2 focus:ring-[#0e00d0] [&>option]:bg-[#0a0a0a] [&>option]:text-white"
                    >
                  ...
                  <motion.div variants={itemVariants} className="col-span-2 flex justify-end">
                    <motion.button
                      ...
                      className="bg-[#0e00d0] cursor-pointer px-4 py-2 rounded-xl hover:bg-blue-700 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {isSubmitting ? (
                        <motion.div
                          animate={{ rotate: 360 }}
              ...
              {abaAtiva === "dadosAcesso" && (
                ...
                >
                  <motion.div variants={itemVariants}>
                    ...
                    <p className="text-white">miltonjunio9x2@gmail.com</p>
                  </motion.div>
                  <motion.div variants={itemVariants}>
