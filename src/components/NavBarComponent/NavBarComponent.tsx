...
import { DynamicTopFolder } from "../layout/DynamicTopFolder";
...
export const NavBarComponent = () => {
  ...
  const navItems = [
    { href: "/home", label: "Início", icon: Home },
    {
      ...
      isActive: (path: string) =>
        path.includes("/conteudo") || path.includes("/modulos"),
    },
  ...
  return (
    <>
          ...
          {navItems.map(({ href, label, icon: Icon, isActive }) => {
            ...
            return (
              <button
                ...
                className={`flex items-center cursor-pointer space-x-2 text-xl group relative transition-all duration-300 ${
                  active
                    ? "font-bold text-black dark:text-white"
                    : "text-[#4A5260] hover:text-black dark:hover:text-white"
                }`}
              >
                <Icon
                  className={`w-6 h-6 transition-colors duration-300 ${
                    active
                      ? "text-[#0E00D0]"
                      : "text-[#4A5260] group-hover:text-[#0E00D0]"
                  }`}
                />
      ...
      {/* Menu Mobile */}
      <div
        className={`lg:hidden fixed inset-0 z-50 transition-all duration-300 ease-in-out ${isMobileMenuOpen ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"}`}
      >
        <div
        ...
        <div
          className={`absolute right-0 top-0 h-full w-72 bg-[#00091A] border-l border-[#555555] shadow-2xl transform transition-transform duration-300 ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}
        >
              ...
              {navItems.map(({ href, label, icon: Icon, isActive }) => {
                ...
                return (
                  <button
                    key={href}
                    onClick={() => handleNavigate(href)}
                    className={`flex items-center w-full cursor-pointer space-x-3 text-lg p-3 rounded-xl transition-all duration-300 group ${
                      active
                        ? "bg-[#1F293C] border border-[#0E00D0] text-white font-bold"
                        : "text-[#4A5260] hover:text-[#0E00D0] hover:bg-[#1A2332]"
                    }`}
                  >
                    <Icon
                      className={`w-6 h-6 transition-colors duration-300 ${
