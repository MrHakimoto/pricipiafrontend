...
export const DuvidaQuestao: React.FC<DuvidaQuestaoProps> = ({
    ...
    // Buscar threads relacionadas quando o componente montar ou questaoId mudar
    useEffect(() => {
        if (questaoId && token) {
    ...
    const carregarThreadsRelacionadas = async () => {
        ...
        try {
            ...
            // Filtrar threads relacionadas a esta questão
            const threadsDaQuestao = response.data.filter(thread =>
                thread.linkable_type === "App\\Models\\Questao" &&
                thread.linkable_id === questaoId
            );
